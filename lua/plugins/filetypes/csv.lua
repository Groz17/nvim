...
return {
  ...
  -- },
  {
    ...
    -- delim w/ regex? old sub trick ig
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('BufEnter',{
        ...
        callback = function (args)
         vim.keymap.set({'i','n'},'<c-c><c-c>',[[<c-\><c-n><cmd>exe "norm \<f5>"<cr>]],{buffer=args.buf })
        end
      })
    end,
  },
  {
    ...
    keys = {
      ...
      { '<s-tab>', "<CMD>lua require('decisive').align_csv_prev_col()<cr>", desc = 'Align CSV prev col', buffer = true, ft = 'csv' },
      { '<tab>', "<CMD>lua require('decisive').align_csv_next_col()<cr>", desc = 'Align CSV next col', buffer = true, ft = 'csv' },
    },
